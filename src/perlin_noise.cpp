...
// http://libnoise.cvs.sourceforge.net/viewvc/libnoise/noise/src/noisegen.cpp?revision=1.8&view=markup

// Initialize with the reference values for the permutation vector
PerlinNoise::PerlinNoise():_seed(1) {
	setP();
}

// Generate a new permutation vector based on the value of seed
PerlinNoise::PerlinNoise(unsigned int seed):_seed(seed) {
	setP();
}

void PerlinNoise::setP()
{
	p = {
...
double PerlinNoise::noise(double x, double y, double z) {
  ...
  zs = fade (z - (double)z0);
	...
	n0   = GradientNoise3D (x, y, z, x0, y0, z0, _seed);
	n1   = GradientNoise3D (x, y, z, x1, y0, z0, _seed);
	ix0  = lerp (n0, n1, xs);
	n0   = GradientNoise3D (x, y, z, x0, y1, z0, _seed);
	n1   = GradientNoise3D (x, y, z, x1, y1, z0, _seed);
	ix1  = lerp (n0, n1, xs);
	iy0  = lerp (ix0, ix1, ys);
	n0   = GradientNoise3D (x, y, z, x0, y0, z1, _seed);
	n1   = GradientNoise3D (x, y, z, x1, y0, z1, _seed);
	ix0  = lerp (n0, n1, xs);
	n0   = GradientNoise3D (x, y, z, x0, y1, z1, _seed);
	n1   = GradientNoise3D (x, y, z, x1, y1, z1, _seed);
	ix1  = lerp (n0, n1, xs);
	iy1  = lerp (ix0, ix1, ys);
  ...
  int iy, int iz, int seed)
{
  ...
  double xvGradient = p[(vectorIndex << 2)    ];
  double yvGradient = p[(vectorIndex << 2) + 1];
  double zvGradient = p[(vectorIndex << 2) + 2];
